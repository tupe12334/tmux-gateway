...
use tmux_gateway_core::{
    PaneLayout, RealTmuxExecutor, ResizeDirection, kill_session, list_panes, list_windows,
    new_session, new_window, resize_pane, select_layout, split_window,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let editor_win = new_window(&executor, SESSION_NAME, "editor", None).await?;
    println!("  Created window: {} (index={})", editor_win.name, editor_win.index);

    let server_win = new_window(&executor, SESSION_NAME, "server", None).await?;
    println!("  Created window: {} (index={})", server_win.name, server_win.index);

    // 3. Split windows into panes
    println!("\nSplitting 'editor' window...");
    let editor_target = format!("{SESSION_NAME}:{}.0", editor_win.index);
    ...
    for window in &windows {
        ...
        println!("  {} (index={}) — {} panes:", window.name, window.index, panes.len());
        for pane in &panes {
            println!("    {pane}");
        }
    }
    ...
    // 5. Select layout
    let editor_win_target = format!("{SESSION_NAME}:{}", editor_win.index);
    ...
    // 6. Resize panes
    let resize_target = format!("{SESSION_NAME}:{}.0", editor_win.index);
