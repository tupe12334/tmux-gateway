...
pub async fn create_session_with_windows(
    ...
    if let Some((first, rest)) = window_names.split_first() {
        ...
        if let Err(e) = super::rename_window(&default_window_target, first).await {
            // Rollback: kill the session
            let _ = super::kill_session(name).await;
            return Err(e);
        }

        // Create remaining windows
        for wn in rest {
            if let Err(e) = super::new_window(name, wn).await {
